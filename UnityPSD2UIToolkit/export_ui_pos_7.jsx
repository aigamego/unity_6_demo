...
function startExportEvent(e) {
    ...
    //position and size export 
    exportPosAndImageSizeJson();
...
function exportPosAndImageSizeJson() {
    // 获取当前文档
    ...
    // 递归遍历所有子图层
    function traverseLayers(layers, index) {  // Pass index here
      var currentIndex = index || 0;
        for (var i = 0; i < layers.length; i++) {
            var currentLayer = layers[i];
            if (currentLayer.typename === "ArtLayer") {
                getLayerInfo(currentLayer, currentIndex++);
            } else if (currentLayer.typename === "LayerSet") {
                currentIndex = traverseLayers(currentLayer.layers, currentIndex); // 如果是图层组，递归遍历, and update index
            }
        }
    ...
    var rootObject = {
      ...
      // 保存图层信息为 JSON 文件 (Modified to use exportRoot)
    var jsonFilePath =  exportRoot + "/layers.json"; // Using exportRoot
    var jsonFile = new File(jsonFilePath);  // No longer a save dialog
    ...
    // 创建保存 PNG 图片的文件夹
    // var folderPath = jsonFile.path + "/png_layers"; // Use exportRoot and image folder
      var folderPath = getPathInfoImagesUtil().folderPathFull; // Use the helper function

      if (!folderPath)
      {
        folderPath = exportRoot + "/png_layers";
      }

    var pngFolder = createFolder(folderPath);


  // --- PNG Export Logic (Modified for consistency and visibility handling) ---
    ...
    function exportLayerAsPNG(layer, folder) {
        ...
        if (layer.typename !== "ArtLayer") return; // Only export ArtLayers


        layer.visible = true; // Make the current layer visible

        var pngFile = new File(folder + "/" + getLayerNameUtil({name:layer.name}) + ".png"); // Use getLayerNameUtil
        ...
        layer.visible = false;  // Hide the layer after exporting
    }

    function traverseAndExport(layers, folder){
        for(var i = 0; i < layers.length; i++){
            var currentLayer = layers[i];
            if (currentLayer.typename === "ArtLayer"){
                exportLayerAsPNG(currentLayer, folder);

            } else if(currentLayer.typename === "LayerSet") {
              traverseAndExport(currentLayer.layers, folder); // If it is layer set
          }
  ...
  // Then, export visible layers one by one
  traverseAndExport(doc.layers, pngFolder)
    alert("所有图层已导出到: " + pngFolder);



    } else {
        alert("没有选择文件保存路径!"); // This alert might not be relevant anymore
    }
}