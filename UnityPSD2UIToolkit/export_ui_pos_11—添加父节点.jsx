...
// =====================================================
// 图层信息导出：遍历所有图层收集名称、位置、尺寸和父节点（组名称）
// Layers JSON export: Recursively collect layer name, position, size, and parent group name
// =====================================================
function getLayersInfo(layers, infoArray, indexRef, parentName) {
    // 如果没有传入父节点名称，则默认为空字符串 / Default parent name to empty string if not provided
    parentName = parentName || "";
    for (var i = 0; i < layers.length; i++) {
        var lyr = layers[i];
        if (lyr.isBackgroundLayer) continue;
        if (lyr.typename === "ArtLayer") {
            var b = lyr.bounds; // [left, top, right, bottom]（UnitValue 对象）
            var info = {
                name: lyr.name,
                parent: parentName, // 新增属性：父节点，即图层所在的组 / New property: parent group name
                position: {
            ...
            // 递归调用，将当前组名称作为父节点传递给子图层 / Recursive call: pass the current group name as the parent for child layers
            getLayersInfo(lyr.layers, infoArray, indexRef, lyr.name);
        }
    }
}
...
function exportLayersJSON(doc, exportFolder) {
    ...
    // 初始调用时父节点名称为空 / Initial call with empty parent group name
    getLayersInfo(doc.layers, layersInfo, indexRef, "");
    var jsonObj = { layers: layersInfo };
